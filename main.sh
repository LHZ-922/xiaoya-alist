...
if [ -n "${XIAOYA_BRANCH}" ]; then
        if ! curl -sL "https://raw.githubusercontent.com/LHZ-922/xiaoya-alist/${XIAOYA_BRANCH}/all_in_one.sh" -o /tmp/xiaoya_install.sh; then
            ERROR "脚本获取失败！"
            exit 1
        fi
    fi
else
            if ! curl -sL https://raw.githubusercontent.com/LHZ-922/xiaoya-alist/master/all_in_one.sh -o /tmp/xiaoya_install.sh; then
                ERROR "脚本获取失败！"
                exit 1
            fi
        fi
